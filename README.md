...
外部 POKE 請呼叫 `GET https://linebot-3r2w.onrender.com/api/health/db`。此端點只執行一次 `SELECT 1`，同時確認 Render 與 Aiven MySQL 可用；成功回傳 HTTP 200，資料庫無法連線則回傳 HTTP 503。程式內不再啟動另一個資料庫心跳，也不再用錯誤手機或測試客戶喚醒服務。
...
- 預約結束時間永遠由方案分鐘數計算。
- 一般預約入口都必須至少提前 90 分鐘；09:00 最早可預約 10:30。店長與 Admin 可強制略過，客服可由兩者個別授權。
- 一般帳號不可建立師傅或房間撞期；店長、Admin 及獲授權客服可強制建立並留下稽核紀錄。
- 師傅班表沒有最低時數，使用 24 小時制並可跨日。
- 距離開班 90 分鐘內（含剛好 90 分鐘），師傅不能自行新增或撤銷；店長／Admin 與獲授權客服可強制處理。
- `師傅在線` 與正式班表分開；師傅可輸入「上線／下線」，接單後 Bot 會再次詢問是否下線。
- 健康資料只允許 Admin 讀寫，並在 MySQL 中以加密內容保存。
- 店長與 Admin 可暫時退役或永久刪除師傅；存在營運歷史時永久刪除會被阻止。
- 店長與 Admin 可在後台編輯師傅身高、體重與角色，並以 LINE User ID 串接或撤銷後改綁；只有派單 Bot 測試通知成功送達才會完成串接。
...
- LINE 管理員必須先輸入 `root`，再輸入對應後台 PIN 才會綁定；一般師傅提示不公開此指令。同一 LINE 可同時是師傅與 Root，管理選單可撤銷原師傅連結後改綁目前 LINE，並可由 Admin／Jerry 以 `新增客服 顯示名稱 登入帳號 4位PIN` 建立客服帳號。
- LINE 客戶先看簡易確認 Flex，按下確認才寫入訂單；正式建立後再回傳完整訂單 Flex。
- 備用網頁預約同樣檢查正式班表、師傅撞期、兩間房容量、90 分鐘與重複送出；啟用 LIFF 後由後端驗證 LINE ID Token。
- 客戶名稱與內部識別分開；內部識別使用 `SSR/SR/R/N-手機後四碼`，只供管理端查看。手機號碼是可編輯的客戶 ID，同一客戶可保存多支手機。
...
目前測試包含登入權限、兩間房初始化、預約結束時間、一般帳號撞期阻擋、管理強制權限、店長帳號限制，以及跨日／90 分鐘排班規則。
