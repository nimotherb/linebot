...
import {
  ...
  mapAuditLog,
  mapCustomer,
  type StaffCategoryView,
  mapPromotion,
  mapService,
  mapShift,
  mapStaff,
  type LineNotificationBatch,
  PromotionView,
  RawBookingRequest,
  SpaApi,
  StaffIdentity,
  ...
  type StaffScheduleReminderHistoryBatch,
} from './api-client';
...
type ModalState =
  ...
  | { type: 'staffCategoryList' }
  | { type: 'staffCategory'; id?: number }
  | { type: 'bookingRequest'; id: number }
  | { type: 'user' }
  | { type: 'account' }
  | { type: 'scheduleReminderHistory' }
  | null;

type BulkEntity = 'appointments' | 'booking_requests' | 'shifts' | 'customers' | 'services' | 'promotions' | 'rooms' | 'venues' | 'users' | 'audit_logs';
...
const roomOptionValue = (id: number) => `room:${id}`;
const venueOptionValue = (id: number) => `venue:${id}`;
const venueOptionLabel = (venue: NonNullable<BootstrapData['venues']>[number]) => `${venue.name}${venue.address ? `-${venue.address}` : ''}`;
...
const readStaffCategories = (data: FormData, fallback: string[] = []) => {
  const values = data.getAll('categories').map((value) => String(value).trim()).filter(Boolean);
  return Array.from(new Set(values.length ? values : fallback.filter(Boolean)));
};
...
function StatusPill({ status }: { status: string }) {
  const tone = status === '待確認' ? 'warning' : status === '已完成' || status === '已付款' ? 'done' : status === '已取消' ? 'muted-status' : status === '已停用' ? 'muted-status' : 'confirmed';
...
export default function Home() {
  ...
  const [shiftMode, setShiftMode] = useState<'single' | 'bulk'>('single');
  ...
  const [scheduleReminderHistory, setScheduleReminderHistory] = useState<StaffScheduleReminderHistoryBatch[]>([]);
  ...
  const [lineNotificationHistory, setLineNotificationHistory] = useState<LineNotificationBatch[]>([]);
  const [lineNotificationHistoryLoading, setLineNotificationHistoryLoading] = useState(false);
  const [lineNotificationHistoryError, setLineNotificationHistoryError] = useState('');
  ...
  const categoryName = (key: string) => staffCategories.find((item) => item.key === key)?.name || key;
  ...
  const applyBootstrap = (data: BootstrapData, mode: 'live' | 'staff' = 'live') => {
    ...
    setStaffCategories(data.staff_categories || []);
    ...
    if (mode === 'live' && requestedSection && navGroups.some((group) => group.items.some((item) => item.id === requestedSection))) {
      setActive(requestedSection);
      if (new URLSearchParams(window.location.search).get('action') === 'new' && requestedSection === 'staff') setModal({ type: 'staff' });
    }
  };
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      const directAdminToken = searchParams.get('admin_token');
      if (directAdminToken) {
        try {
          const data = await new SpaApi(directAdminToken).bootstrap();
          if (!activeRequest) return;
          window.sessionStorage.setItem('equalspa-admin-token', directAdminToken);
          window.sessionStorage.removeItem('equalspa-staff-token');
          window.history.replaceState({}, '', `${window.location.pathname}${searchParams.get('section') ? `?section=${encodeURIComponent(searchParams.get('section') || '')}${searchParams.get('action') ? `&action=${encodeURIComponent(searchParams.get('action') || '')}` : ''}` : ''}`);
          setToken(directAdminToken);
  ...
  useEffect(() => {
    if (modal?.type !== 'appointmentDetail' || !selectedAppointment?.apiId) {
      setLineNotificationHistory([]);
      setLineNotificationHistoryError('');
      return;
    }
    let activeRequest = true;
    setLineNotificationHistoryLoading(true);
    setLineNotificationHistoryError('');
    api.getLineNotificationHistory(selectedAppointment.apiId).then((rows) => {
      if (activeRequest) setLineNotificationHistory(rows);
    }).catch(() => {
      if (activeRequest) setLineNotificationHistoryError('通知歷程暫時無法取得');
    }).finally(() => {
      if (activeRequest) setLineNotificationHistoryLoading(false);
    });
    return () => { activeRequest = false; };
  }, [api, modal?.type, selectedAppointment?.apiId]);
  ...
  const currentStaffMember = staffIdentity ? staff.find((item) => item.apiId === staffIdentity.id) : undefined;
  ...
  const filteredScheduleStaff = useMemo(() => staff.filter((member) => (
    ...
    && (scheduleCategoryFilter === '全部' || (member.categories || [member.category]).includes(scheduleCategoryFilter as StaffMember['category']))
  ...
  const dispatchScheduleReminders = async (staffIds: number[] = selectedScheduleStaffIds) => {
    if (scheduleReminderBusy || staffIds.length === 0) return;
    if (!window.confirm('此操作會消耗 LINE 群發訊息額度，確定要派發嗎？')) return;
    setScheduleReminderBusy(true);
    try {
      const result = await api.dispatchStaffScheduleReminders(staffIds);
      setScheduleReminderResult(result);
      setSelectedScheduleStaffIds([]);
    } catch (error) {
      const message = error instanceof Error ? error.message : '';
      notify(message.includes('LINE 月額度已用完') ? 'LINE 月額度已用完' : 'LINE 推播未完成');
    } finally {
      setScheduleReminderBusy(false);
    }
  };

  const loadScheduleReminderHistory = async () => {
    if (scheduleReminderHistoryLoading) return;
    setScheduleReminderHistoryLoading(true);
    try {
      setScheduleReminderHistory(await api.getStaffScheduleReminderHistory());
      setModal({ type: 'scheduleReminderHistory' });
    } catch {
      notify('排班通知紀錄暫時無法取得。');
    } finally {
      setScheduleReminderHistoryLoading(false);
    }
  };
  ...
  const addAppointment = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const locationSelection = String(data.get('room') || 'pending');
    const roomId = locationSelection.startsWith('room:') ? Number(locationSelection.slice(5)) : null;
    const venueId = locationSelection.startsWith('venue:') ? Number(locationSelection.slice(6)) : null;

    if (appMode === 'live' || appMode === 'staff') {
      try {
        const roomRecord = roomId ? rooms.find((item) => item.id === roomId) : undefined;
        const venueRecord = venueId ? venues.find((item) => item.id === venueId) : undefined;
        const promotionIds = data.getAll('promotionId').map((value) => Number(value)).filter(Boolean);
        const endHour = Number(end.split(':')[0]);
        const startHour = Number(start.split(':')[0]);
        let endDate = date;
        if (endHour < 24 && Number.isFinite(startHour) && endHour < startHour) {
        ...
        const payload = {
          ...
          end_time: end ? `${endDate}T${end}:00` : undefined,
          staff_id: selectedStaffMember?.apiId || null,
          room_id: roomRecord?.id || null,
          venue_id: venueRecord?.id || null,
          promotion_id: promotionIds[0] || null,
          promotion_ids: promotionIds,
          is_admin_override: canOverrideTimeRules,
  ...
  const addShift = async (event: FormEvent<HTMLFormElement>, origin: 'admin' | 'staff') => {
    ...
    if (mode === 'bulk') {
      ...
      try {
        ...
        notify(error instanceof Error ? error.message : '批量排班建立失敗');
      }
      return;
    }
  ...
  const saveAppointmentEdit = async (event: FormEvent<HTMLFormElement>, appointment: Appointment) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      staff_id: staffValue ? Number(staffValue) : null,
      room_id: roomRecord?.id || null,
      venue_id: venueRecord?.id || null,
      location_type: roomRecord ? 'onsite' : venueRecord ? 'external' : 'pending',
      status: String(data.get('status')) === '取消訂單' ? '已取消' : String(data.get('status')),
      notes: String(data.get('note') || ''),
  ...
  const saveCustomer = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const updated = await api.updateCustomer(selectedCustomer.apiId, {
        ...
        birthday: String(data.get('birthday') || '').trim() || null,
      });
  ...
  const resolveCustomerBirthday = async (accept: boolean) => {
    if (!selectedCustomer?.apiId || !canManageAll || !selectedCustomer.birthdayPending) return;
    try {
      const updated = accept ? await api.confirmCustomerBirthday(selectedCustomer.apiId) : await api.rejectCustomerBirthday(selectedCustomer.apiId);
      const mapped = mapCustomer(updated);
      setCustomers((current) => current.map((item) => item.id === selectedCustomer.id ? mapped : item));
      notify(accept ? '生日變更已確認。' : '生日變更已拒絕。');
    } catch (error) {
      notify(error instanceof Error ? error.message : '生日變更處理失敗');
    }
  };
  ...
  const addStaff = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const categories = readStaffCategories(data, staffCategories[0]?.key ? [staffCategories[0].key] : []);
    const category = categories[0] || '';
    if (appMode === 'live') {
      try {
        let created = await api.createStaff({ name, category, categories, line_user_id: String(data.get('lineUserId') || '') || null, phone: String(data.get('phone') || '') || null, photo_url: String(data.get('photoUrl') || '').trim() || null, height: Number(data.get('height') || 0) || null, weight: Number(data.get('weight') || 0) || null, role: String(data.get('role') || '') || null, bio: String(data.get('bio') || '').trim().slice(0, 60) || null });
  ...
  const saveStaffProfile = async (event: FormEvent<HTMLFormElement>, member: typeof staff[number]) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const categories = readStaffCategories(data, member.categories || [member.category]);
    const category = categories[0] || member.category;
    const photo = data.get('photoFile');
    try {
      if (appMode === 'live' && member.apiId) {
        let updated = await api.updateStaff(member.apiId, {
          ...
          bio: String(data.get('bio') || '').trim().slice(0, 60) || null,
        });
        ...
        setStaff((current) => current.map((item) => item.id === member.id ? { ...item, name: String(data.get('name')).trim(), category, categories, photoUrl: uploaded || undefined, height: String(data.get('height') || '') || undefined, weight: String(data.get('weight') || '') || undefined, role: String(data.get('role') || '') as StaffMember['role'] || undefined, bio: String(data.get('bio') || '').trim().slice(0, 60) || undefined } : item));
      }
      setModal(null);
  ...
  const saveSettlement = async (event: FormEvent<HTMLFormElement>, appointment: Appointment) => {
    event.preventDefault();
    if (!appointment.apiId || !canManageAll) return;
    const data = new FormData(event.currentTarget);
    const adjustment = {
      discount_employee_amount: Number(data.get('discountEmployeeAmount') || 0),
      discount_shop_amount: Number(data.get('discountShopAmount') || 0),
      surcharge_employee_amount: Number(data.get('surchargeEmployeeAmount') || 0),
      surcharge_shop_amount: Number(data.get('surchargeShopAmount') || 0),
    };
    const finalAmounts = {
      staff_return_amount: Number(data.get('staffReturnAmount') || 0),
      shop_recovery_amount: Number(data.get('shopRecoveryAmount') || 0),
    };
    const previousAdjustments = [appointment.discountEmployeeAmount, appointment.discountShopAmount, appointment.surchargeEmployeeAmount, appointment.surchargeShopAmount].map((value) => Number(value || 0));
    const adjustmentsChanged = Object.values(adjustment).some((value, index) => value !== previousAdjustments[index]);
    const payload: Record<string, unknown> = { ...adjustment, is_admin_override: true };
    // Leaving calculated totals untouched while changing adjustment fields means
    // the API should recalculate them. Explicitly edited totals remain overrides.
    const previousFinals = { staff_return_amount: Number(appointment.staffReturnAmount || 0), shop_recovery_amount: Number(appointment.shopRecoveryAmount || 0) };
    for (const [key, value] of Object.entries(finalAmounts)) {
      if (!adjustmentsChanged || value !== previousFinals[key as keyof typeof previousFinals]) payload[key] = value;
    }
    try {
      const updated = await api.updateAppointment(appointment.apiId, payload);
      setAppointments((current) => current.map((item) => item.id === appointment.id ? mapAppointment(updated) : item));
      ...
      notify(error instanceof Error ? error.message : '回帳設定更新失敗');
    }
  };
  ...
  const saveStaffCategory = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!canManageAll || appMode !== 'live') return notify('目前未連線至資料庫，不能更新分類。');
    const data = new FormData(event.currentTarget);
    const key = String(data.get('key') || '').trim();
    const name = String(data.get('name') || '').trim();
    const sort_order = Number(data.get('sortOrder') || 0);
    const active = data.get('active') === 'on';
    try {
      const categoryId = modal?.type === 'staffCategory' ? modal.id : undefined;
      const saved = categoryId
        ? await api.updateStaffCategory(categoryId, { key, name, sort_order, active })
        : await api.createStaffCategory({ key, name, sort_order, active });
      setStaffCategories((current) => categoryId ? current.map((item) => item.id === categoryId ? saved : item).sort((a, b) => a.sort_order - b.sort_order || a.id - b.id) : [...current, saved].sort((a, b) => a.sort_order - b.sort_order || a.id - b.id));
      setModal({ type: 'staffCategoryList' });
      notify('員工分類已同步。');
    } catch (error) {
      notify(error instanceof Error ? error.message : '分類更新失敗');
    }
  };

  const deleteStaffCategory = async (category: StaffCategoryView) => {
    if (!canManageAll || !window.confirm(`確定刪除分類「${category.name}」？員工與歷史訂單會保留。`)) return;
    try {
      await api.deleteStaffCategory(category.id);
      setStaffCategories((current) => current.filter((item) => item.id !== category.id));
      setStaff((current) => current.map((member) => ({ ...member, categories: (member.categories || []).filter((key) => key !== category.key), category: member.category === category.key ? '' : member.category })));
      notify('分類已刪除，員工與歷史資料保留。');
    } catch (error) {
      notify(error instanceof Error ? error.message : '分類刪除失敗');
    }
  };

  const requestStaffPhoneChange = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!isStaffUser) return;
    const data = new FormData(event.currentTarget);
    try {
      const updated = await api.requestStaffPhoneChange(String(data.get('phone') || '').trim());
      setStaff((current) => current.map((item) => item.apiId === updated.id ? mapStaff(updated) : item));
      setModal(null);
      notify('手機 ID 修改申請已送出，待店長或 Admin 審核。');
    } catch (error) {
      notify(error instanceof Error ? error.message : '手機 ID 修改申請失敗');
    }
  };

  const reviewStaffPhoneChange = async (member: StaffMember, approve: boolean) => {
    if (!member.apiId || !canManageAll || !member.phoneChangePending) return;
    try {
      const updated = approve ? await api.approveStaffPhoneChange(member.apiId) : await api.rejectStaffPhoneChange(member.apiId);
      setStaff((current) => current.map((item) => item.id === member.id ? mapStaff(updated) : item));
      notify(approve ? `${member.name} 的手機 ID 已核准。` : `${member.name} 的手機 ID 申請已駁回。`);
    } catch (error) {
      notify(error instanceof Error ? error.message : '手機 ID 申請處理失敗');
    }
  };
  ...
  const renderAppointments = () => (
    ...
    {appMode === 'live' && <section className="panel table-panel booking-request-panel">
      ...
      <div className="booking-request-list">
        {bookingRequests.filter((item) => item.status === 'pending').map((item) => <article className="booking-request-card" key={item.id}>
          <label className="selection-check"><input type="checkbox" checked={(selectedIds.booking_requests || []).includes(item.id)} onChange={() => toggleSelected('booking_requests', item.id)} /><span>選取</span></label><div><strong>{item.request_id}・{item.customer_name}</strong><small>{item.customer_serial ? `客人識別 ${item.customer_serial}・` : ''}{item.start_time.replace('T', ' ').slice(0, 16)}・{item.staff_name}・{item.service_name}</small><span>這是預約通知，尚未占用師傅或房間。</span></div>
    ...
    <section className="panel table-panel">
      <div className="toolbar">
        ...
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>{['全部', '待確認', '已確認'].map((status) => <option key={status} value={status}>{status}</option>)}</select>
      ...
      <div className="data-table appointment-table">
        ...
        {filteredAppointments.map((item) => (
          <div className="table-row interactive" role="button" tabIndex={0} key={item.id} onClick={() => setModal({ type: 'appointmentDetail', id: item.id })}>
            ...
            <span><strong>{item.customer}</strong><small>{item.customerSerial ? `客人識別 ${item.customerSerial}・` : ''}{item.phone}</small></span>
  ...
  const renderSchedule = () => {
    ...
    const visibleShiftIds = shifts
      .filter((item) => days.some((day) => shiftSegmentsForDay(item, day.date).length > 0) && (item.staffId ? visibleStaffIds.has(item.staffId) : visibleStaffNames.has(item.staff)))
  ...
  const renderOperations = () => {
    const columns: Array<{ key: Appointment['status']; label: string }> = [{ key: '待確認', label: '待確認' }, { key: '已確認', label: '已確認' }];
  ...
  const renderCheckout = () => (
    <section className="panel table-panel checkout-history">
      <div className="panel-heading"><div><p className="eyebrow">COMPLETED ORDERS</p><h2>完成訂單（含取消）</h2></div><span className="room-count">{completedAppointments.length}</span></div>
      <div className="checkout-history-list">
        {completedAppointments.map((item) => <article className="checkout-row completed-history-row interactive" key={item.id} onClick={() => setModal({ type: 'appointmentDetail', id: item.id })}>
        ...
        {completedAppointments.length === 0 && <div className="empty-state">目前沒有已完成或已取消的訂單。</div>}
      </div>
    </section>
  );
  ...
  const renderStaff = () => (
    <section className="panel table-panel">
      ...
      <div className="staff-card-grid">{pagedStaff.map((member) => <article className={member.status === '暫時退役' ? 'staff-card retired' : 'staff-card'} key={member.id}>
        ...
        <div className="staff-meta"><span>手機 ID：{member.phone || '未設定'}</span><span>{member.lineConnected ? 'LINE 已串接' : 'LINE 待串接'}</span><span>{member.isOnline ? '目前上線' : '目前下線'}</span><span>{isViewer || isStaffUser ? '公開基本資料' : '私密資料已分離'}</span></div>
        {canManageAll && member.phoneChangePending && <div className="form-note">待審核手機 ID：{member.requestedPhone}<div className="staff-card-actions"><button className="text-button" onClick={() => reviewStaffPhoneChange(member, true)}>核准</button><button className="danger-text" onClick={() => reviewStaffPhoneChange(member, false)}>駁回</button></div></div>}
        {/* 回帳表選擇已移除；訂單建立時依快照與彈性金額欄位自動計算。 */}
  ...
  return (
    <main className="app-shell">
      ...
      {modal?.type === 'account' && <Modal title="登入資訊" subtitle={identity ? '可自行修改顯示名稱、登入帳號與數字 PIN。' : isStaffUser ? '師傅使用手機 ID 登入；如需變更可送出申請。' : '師傅只使用手機 ID；客服與管理者使用帳號及 PIN。'} onClose={() => setModal(null)} wide>{identity ? <form className="modal-form" onSubmit={saveOwnAccount}><div className="form-grid two"><label>顯示名稱<input name="displayName" defaultValue={identity.display_name} required /></label><label>登入帳號<input name="username" defaultValue={identity.username} required autoCapitalize="none" /></label><label>目前 PIN<input name="currentPin" required type="password" inputMode="numeric" pattern="[0-9]+" minLength={4} /></label><label>新 PIN（不修改可留空）<input name="newPin" type="password" inputMode="numeric" pattern="[0-9]*" minLength={4} maxLength={12} /></label><label>再次輸入新 PIN<input name="confirmPin" type="password" inputMode="numeric" pattern="[0-9]*" minLength={4} maxLength={12} /></label></div><div className="form-note">儲存後會撤銷目前工作階段，請使用新資料重新登入。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">更新帳密</button></footer></form> : isStaffUser && staffIdentity ? <form className="modal-form" onSubmit={requestStaffPhoneChange}><label>目前手機 ID<input value={currentStaffMember?.phone || '未設定'} readOnly /></label><label>申請新的手機 ID<input name="phone" required inputMode="tel" pattern="09[0-9]{8}" defaultValue={currentStaffMember?.phone || ''} placeholder="09xxxxxxxx" /></label>{currentStaffMember?.phoneChangePending && <div className="form-note">待審核申請：{currentStaffMember.requestedPhone}</div>}<div className="form-note">送出後由店長或 Admin 審核，核准前仍使用目前手機 ID 登入。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">送出修改申請</button></footer></form> : <><div className="account-login-grid"><form className="modal-form account-login-card" onSubmit={login}><p className="eyebrow">MANAGEMENT</p><h3>客服／店長／Admin</h3><label>登入帳號<input name="username" required autoCapitalize="none" autoComplete="username" placeholder="例如：admin" /></label><label>數字 PIN<input name="pin" required type="password" inputMode="numeric" pattern="[0-9]+" minLength={4} autoComplete="current-password" /></label><button className="primary-button full" disabled={loginBusy}>{loginBusy ? '登入中…' : '管理帳號登入'}</button></form><form className="modal-form account-login-card" onSubmit={loginStaff}><p className="eyebrow">STAFF</p><h3>師傅手機 ID 登入</h3><label>手機 ID<input name="staffPhone" required inputMode="tel" pattern="09[0-9]{8}" autoComplete="tel" placeholder="09xxxxxxxx" /></label><button className="secondary-button full" disabled={loginBusy}>{loginBusy ? '登入中…' : '以員工身分進入'}</button></form></div>{loginError && <div className="auth-error modal-auth-error">{loginError}</div>}<div className="form-note">從 LINE Bot 開啟師傅後台時會以安全連結直接登入，不會再出現登入畫面。</div></>}</Modal>}
      ...
      {modal?.type === 'appointment' && <Modal title="新增預約" subtitle={canOverrideTimeRules ? '目前帳號可略過提前時間及師傅／房間撞期限制。' : '一般帳號最少提前 90 分鐘，並檢查師傅與房間衝突。'} onClose={() => setModal(null)} wide><form className="modal-form" onSubmit={addAppointment}><div className="form-grid two"><label>客戶姓名<input name="customer" required placeholder="例如：王先生" /></label><label>手機號碼<input name="phone" required inputMode="numeric" pattern="09[0-9]{8}" placeholder="09xxxxxxxx" /></label><label>日期<input name="date" type="date" min={canOverrideTimeRules ? undefined : todayIso} defaultValue={bookingDefault.date} required /></label><label>開始時間<input name="start" type="text" inputMode="numeric" pattern="(?:[01][0-9]|2[0-3]):[0-5][0-9]" defaultValue={bookingDefault.time} placeholder="09:00" required /></label><label>結束時間<input name="end" type="text" inputMode="numeric" pattern="(?:[01][0-9]|2[0-3]|[2-4][0-8]):[0-5][0-9]" placeholder="11:00 或 27:00" required /></label><label>服務方案<select name="serviceId" defaultValue={plans.find((item) => item.code === 'C')?.id}>{plans.filter((item) => item.active).map((plan) => <option value={plan.id} key={plan.id}>{plan.code}・{plan.name}｜{plan.duration} 分｜{formatCurrency(plan.price)}</option>)}</select></label><fieldset className="checkbox-field"><legend>優惠（可複選）</legend>{promotions.filter((item) => item.active).map((item) => <label key={item.id}><input type="checkbox" name="promotionId" value={item.apiId} />{item.name}</label>)}{promotions.filter((item) => item.active).length === 0 && <span>目前沒有可用優惠</span>}</fieldset><label>指派師傅<select name="staff" defaultValue=""><option value="">未指定（由店長安排）</option>{staff.filter((item) => item.status === '在職').map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>場地／房間<select name="room" defaultValue={rooms[0] ? roomOptionValue(rooms[0].id) : 'pending'}>{rooms.map((room) => <option key={roomOptionValue(room.id)} value={roomOptionValue(room.id)}>{room.name}</option>)}{venues.filter((venue) => venue.active).map((venue) => <option key={venueOptionValue(venue.id)} value={venueOptionValue(venue.id)}>{venueOptionLabel(venue)}</option>)}<option value="pending">待確認</option></select></label><label className="span-two">客服備註<textarea name="note" rows={3} placeholder="不公開給客戶的內部備註" /></label></div><div className="form-note">時間採 24 小時制；跨日請輸入至 48:00，例如隔日凌晨 3 點輸入 27:00。{canOverrideTimeRules ? ' 這筆訂單會以管理強制權限建立，仍會留下稽核紀錄。' : ' 後端也會阻止重疊或過近的時間。'}</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button" type="submit">建立預約</button></footer></form></Modal>}

      {modal?.type === 'appointmentDetail' && selectedAppointment && <Modal title={selectedAppointment.id} subtitle={`${selectedAppointment.date}　${selectedAppointment.start}–${selectedAppointment.end}`} onClose={() => setModal(null)}><div className="detail-stack"><div className="detail-hero"><div><span>客戶</span><strong>{selectedAppointment.customer}</strong><small>{selectedAppointment.customerSerial ? `客人識別 ${selectedAppointment.customerSerial}・` : ''}{selectedAppointment.phone}</small></div><StatusPill status={selectedAppointment.status} /></div><dl><div><dt>師傅</dt><dd>{selectedAppointment.staff}</dd></div><div><dt>服務方案</dt><dd>{selectedAppointment.service}</dd></div><div><dt>優惠</dt><dd>{selectedAppointment.promotionIds?.length ? '優惠' : '未使用'}</dd></div><div><dt>場地</dt><dd>{selectedAppointment.location === '已取消' ? '已取消' : `${selectedAppointment.location}・${selectedAppointment.room}`}</dd></div><div><dt>應收金額</dt><dd>{formatCurrency(selectedAppointment.total)}</dd></div><div><dt>付款狀態</dt><dd>{selectedAppointment.payment}</dd></div></dl>{selectedAppointment.note && <div className="detail-note"><strong>客服備註</strong><p>{selectedAppointment.note}</p></div>}<section className="detail-note line-history"><strong>LINE 通知批次歷程</strong>{lineNotificationHistoryLoading && <p>讀取中…</p>}{lineNotificationHistoryError && <p>{lineNotificationHistoryError}</p>}{!lineNotificationHistoryLoading && !lineNotificationHistoryError && lineNotificationHistory.length === 0 && <p>尚無通知批次。</p>}{lineNotificationHistory.map((batch) => <article key={batch.id} className={`line-history-item line-history-${batch.trigger_type}`}><div><strong>第{batch.dispatch_sequence}次｜{batch.trigger_label}</strong><span>{batch.status_label}</span></div><small>{batch.created_at || '時間未提供'}・成功 {batch.sent_count}・失敗 {batch.failed_count}</small>{batch.failed_kinds.length > 0 && <small>失敗對象：{batch.failed_kinds.map((item) => `${item.label} × ${item.count}`).join('、')}</small>}</article>)}</section><div className="status-actions">{(selectedAppointment.status === '已完成' || selectedAppointment.status === '已取消' ? canManageAll : canEditAppointments) && <button onClick={() => setModal({ type: 'appointmentEdit', id: selectedAppointment.id })}>編輯整張訂單</button>}{canManageAll && <button onClick={() => setModal({ type: 'settlement', id: selectedAppointment.id })}>編輯回帳調整</button>}{canEditAppointments && selectedAppointment.apiId && <button onClick={async () => { try { const result = await api.notifyAppointmentLine(selectedAppointment.apiId!); const history = await api.getLineNotificationHistory(selectedAppointment.apiId!); setLineNotificationHistory(history); notify(`LINE 派發完成：已發送 ${result.sent}、略過 ${result.skipped}、失敗 ${result.failed}`); } catch (error) { notify(error instanceof Error ? error.message : 'LINE 派發失敗'); } }}>LINEBot 派發訂單卡片</button>}{canEditAppointments && selectedAppointment.status !== '已完成' && selectedAppointment.status !== '已取消' && <button onClick={() => setModal({ type: 'checkout', id: selectedAppointment.id })}>記錄付款並完成</button>}</div></div></Modal>}

      {modal?.type === 'appointmentEdit' && selectedAppointment && <Modal title={`編輯 ${selectedAppointment.id}`} subtitle={canManageAll ? '店長與 Admin 可修改整張訂單所有欄位。' : '客服可調整預約與服務內容，金額需由店長或 Admin 處理。'} onClose={() => setModal(null)} wide><form className="modal-form" onSubmit={(event) => saveAppointmentEdit(event, selectedAppointment)}><div className="form-grid two"><label>客戶姓名<input name="customer" defaultValue={selectedAppointment.customer} required /></label><label>手機號碼<input name="phone" defaultValue={selectedAppointment.phone} required /></label><label>日期<input name="date" type="date" defaultValue={selectedAppointment.date} required /></label><label>開始時間<input name="start" type="text" inputMode="numeric" pattern="(?:[01][0-9]|2[0-3]):[0-5][0-9]" defaultValue={selectedAppointment.start} required /></label><label>結束時間<input name="end" type="text" inputMode="numeric" pattern="(?:[01][0-9]|2[0-3]|[2-4][0-8]):[0-5][0-9]" defaultValue={selectedAppointment.end} placeholder="11:00 或 27:00" required /></label><label>服務方案<select name="serviceId" defaultValue={selectedAppointment.serviceId}>{plans.map((plan) => <option key={plan.id} value={plan.apiId}>{plan.code}・{plan.name}</option>)}</select></label><fieldset className="checkbox-field"><legend>優惠（可複選）</legend>{promotions.map((promotion) => <label key={promotion.id}><input type="checkbox" name="promotionId" value={promotion.apiId} defaultChecked={selectedAppointment.promotionIds?.includes(promotion.apiId) || selectedAppointment.promotionId === promotion.apiId} />{promotion.name}</label>)}{promotions.length === 0 && <span>目前沒有可用優惠</span>}</fieldset><label>指派師傅<select name="staffId" defaultValue={selectedAppointment.staffId}>{staff.filter((item) => item.status === '在職').map((item) => <option key={item.id} value={item.apiId}>{item.name}</option>)}</select></label><label>場地／房間<select name="room" defaultValue={selectedAppointment.venueId ? venueOptionValue(selectedAppointment.venueId) : selectedAppointment.roomId ? roomOptionValue(selectedAppointment.roomId) : 'pending'}>{rooms.map((room) => <option key={roomOptionValue(room.id)} value={roomOptionValue(room.id)}>{room.name}</option>)}{venues.filter((venue) => venue.active || venue.id === selectedAppointment.venueId).map((venue) => <option key={venueOptionValue(venue.id)} value={venueOptionValue(venue.id)}>{venueOptionLabel(venue)}</option>)}<option value="pending">待確認</option></select></label><label>訂單狀態<select name="status" defaultValue={selectedAppointment.status}>{['待確認', '已確認', '已完成', '已取消'].map((status) => <option key={status} value={status}>{status === '已取消' ? '取消訂單' : status}</option>)}</select></label>{canManageAll && <><label>原價<input name="basePrice" type="number" min="0" defaultValue={selectedAppointment.basePrice || 0} /></label><label>折扣<input name="discountAmount" type="number" min="0" defaultValue={selectedAppointment.discountAmount || 0} /></label><label>加價<input name="extraAmount" type="number" min="0" defaultValue={selectedAppointment.extraAmount || 0} /></label><label>訂單總額<input name="totalAmount" type="number" min="0" defaultValue={selectedAppointment.total} /></label><label>師傅抽成<input name="commissionAmount" type="number" min="0" defaultValue={selectedAppointment.commissionAmount || 0} /></label></>}<label className="span-two">客服備註<textarea name="note" rows={3} defaultValue={selectedAppointment.note || ''} /></label><label className="span-two">修改原因<input name="reason" placeholder="例如：客戶改期、人工修正優惠" /></label></div><div className="form-note">時間採 24 小時制；跨日請輸入至 48:00，例如隔日凌晨 3 點輸入 27:00。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">儲存訂單</button></footer></form></Modal>}
      ...
      {modal?.type === 'shiftMore' && <Modal className="schedule-more-modal" title={`${modal.staffName}・${modal.day} 排班`} subtitle={`完整清單，共 ${selectedShiftMore.length} 筆`} onClose={() => setModal(null)} wide><div className="schedule-more-list">{selectedShiftMore.map((shift) => <button className={`${isShiftLocked(shift) ? 'shift-card locked' : 'shift-card'}${shift.isNextDay || shift.endDate !== shift.date ? ' cross-day' : ''}${shift.modifiedByAdminId ? ' admin-modified' : ''}`} key={shift.id} onClick={() => (canManageShifts || isStaffUser) && setModal({ type: 'shiftDetail', id: shift.id, origin: isStaffUser ? 'staff' : 'admin' })}>{canManageAll && shift.apiId && <label className="selection-check" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={(selectedIds.shifts || []).includes(shift.apiId)} onChange={() => toggleSelected('shifts', shift.apiId!)} /><span>選取</span></label>}<strong>{shiftTimeLabel(shift)}</strong><small>{shift.modifiedByAdminName ? `管理者：${shift.modifiedByAdminName}` : shift.isNextDay || shift.endDate !== shift.date ? '跨天排班' : shift.source}</small></button>)}</div></Modal>}

      {modal?.type === 'checkout' && selectedAppointment && <Modal title="完成訂單" subtitle={`${selectedAppointment.id}・${selectedAppointment.customer}`} onClose={() => setModal(null)}><form className="modal-form" onSubmit={(event) => finishCheckout(event, selectedAppointment.id)}><div className="receipt"><div><span>{selectedAppointment.service}</span><strong>{formatCurrency(selectedAppointment.basePrice || selectedAppointment.total)}</strong></div><div><span>優惠</span><strong>− {formatCurrency(selectedAppointment.discountAmount || 0)}</strong></div><div><span>其他加價</span><strong>＋ {formatCurrency(selectedAppointment.extraAmount || 0)}</strong></div><div className="receipt-total"><span>應收總額</span><strong>{formatCurrency(selectedAppointment.total)}</strong></div></div><label>付款方式<select name="paymentMethod" defaultValue="現金"><option>現金</option><option>轉帳</option></select></label><label>完成備註<textarea name="note" rows={3} placeholder="例如：現金已收妥" /></label><div className="form-note">按下完成後即視為款項已回帳，不再需要師傅或第三人另外確認。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>稍後處理</button><button className="primary-button" type="submit">確認完成訂單</button></footer></form></Modal>}
      ...
      {modal?.type === 'staff' && <Modal title="新增員工" subtitle="基本資料與 LINE 串接都會保存到正式資料庫。" onClose={() => setModal(null)}><form className="modal-form" onSubmit={addStaff}><label>姓名／稱呼<input name="name" required /></label><label>手機 ID<input name="phone" inputMode="tel" placeholder="師傅手機 ID 登入使用" /></label><fieldset className="checkbox-field"><legend>員工分類（可複選）</legend>{staffCategories.length ? staffCategories.map((item, index) => <label key={item.key}><input name="categories" type="checkbox" value={item.key} defaultChecked={index === 0} />{item.name}</label>) : <span className="empty-state">目前沒有可用分類。</span>}</fieldset><div className="form-grid two"><label>身高（cm）<input name="height" type="number" min="100" max="250" /></label><label>體重（kg）<input name="weight" type="number" min="30" max="250" /></label></div><label>角色<select name="role" defaultValue=""><option value="">尚未設定</option><option>攻擊手</option><option>守備方</option><option>無特定</option><option>攻守兼備</option></select></label><label>師傅自我簡介（最多 60 字）<textarea name="bio" maxLength={60} rows={3} placeholder="例如：細心掌握節奏，讓每次互動都更自在。" /></label><label>公開照片網址<input name="photoUrl" type="url" placeholder="https://..." /></label><label>或從電腦上傳照片<input name="photoFile" type="file" accept="image/jpeg,image/png,image/webp" /></label><div className="form-note">若同時填網址並選擇檔案，會以上傳檔案為準；限 JPEG、PNG、WebP，最大 3 MB。</div>{/* 回帳表選擇已移除，訂單建立時使用快照與彈性金額欄位。 */}<label>LINE User ID（選填）<input name="lineUserId" pattern="U[0-9a-fA-F]{32}" minLength={33} maxLength={33} placeholder="U 開頭的 33 位 LINE User ID" /></label><div className="form-note">若填寫 LINE ID，只有測試通知成功送達才會建立並完成串接。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">建立員工</button></footer></form></Modal>}

      {modal?.type === 'staffCategoryList' && <Modal title="員工分類" subtitle="分類名稱、排序與啟用狀態由資料庫管理。" onClose={() => setModal(null)}><div className="settings-list">{staffCategories.map((item) => <article key={item.id}><div><strong>{item.name}</strong><small>{item.key}・排序 {item.sort_order}・{item.active ? '啟用' : '停用'}</small></div><div className="inline-actions"><button className="text-button" onClick={() => setModal({ type: 'staffCategory', id: item.id })}>編輯</button><button className="danger-button" onClick={() => deleteStaffCategory(item)}>刪除分類</button></div></article>)}{staffCategories.length === 0 && <div className="empty-state">目前沒有可用分類。</div>}<footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal({ type: 'staffCategory' })}>新增分類</button><button type="button" className="secondary-button" onClick={() => setModal(null)}>關閉</button></footer></div></Modal>}
      {modal?.type === 'staffCategory' && (() => { const item = modal.id ? staffCategories.find((category) => category.id === modal.id) : undefined; return <Modal title={item ? `編輯分類 ${item.name}` : '新增分類'} subtitle="分類刪除只移除關聯，不會刪除員工或歷史訂單。" onClose={() => setModal({ type: 'staffCategoryList' })}><form className="modal-form" onSubmit={saveStaffCategory}><label>穩定 key<input name="key" required pattern="[A-Za-z0-9_-]+" defaultValue={item?.key || ''} /></label><label>顯示名稱<input name="name" required defaultValue={item?.name || ''} /></label><label>排序<input name="sortOrder" type="number" min="0" defaultValue={item?.sort_order || 0} /></label><label className="checkbox-field"><input name="active" type="checkbox" defaultChecked={item?.active ?? true} />啟用</label><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal({ type: 'staffCategoryList' })}>取消</button><button className="primary-button">儲存分類</button></footer></form></Modal>; })()}

      {modal?.type === 'staffEdit' && (() => { const member = staff.find((item) => item.id === modal.id); const memberCategories = member?.categories || (member?.category ? [member.category] : []); return member ? <Modal title={`編輯 ${member.name}`} subtitle="可編輯手機 ID、身高、體重、角色及撤銷後重新串接 LINE。" onClose={() => setModal(null)}><form className="modal-form" onSubmit={(event) => saveStaffProfile(event, member)}><label>姓名／稱呼<input name="name" defaultValue={member.name} required /></label><label>手機 ID<input name="phone" defaultValue={member.phone || ''} inputMode="tel" placeholder="09xxxxxxxx" /></label><fieldset className="checkbox-field"><legend>員工分類（可複選）</legend>{staffCategories.length ? staffCategories.map((item) => <label key={item.key}><input name="categories" type="checkbox" value={item.key} defaultChecked={memberCategories.includes(item.key)} />{item.name}</label>) : <span className="empty-state">目前沒有可用分類。</span>}</fieldset><div className="form-grid two"><label>身高（cm）<input name="height" type="number" min="100" max="250" defaultValue={member.height || ''} /></label><label>體重（kg）<input name="weight" type="number" min="30" max="250" defaultValue={member.weight || ''} /></label></div><label>角色<select name="role" defaultValue={member.role || ''}><option value="">尚未設定</option><option>攻擊手</option><option>守備方</option><option>無特定</option><option>攻守兼備</option></select></label><label>師傅自我簡介（最多 60 字）<textarea name="bio" maxLength={60} rows={3} defaultValue={member.bio || ''} placeholder="例如：細心掌握節奏，讓每次互動都更自在。" /></label>{member.photoUrl && <img className="staff-edit-preview" src={member.photoUrl} alt={`${member.name}目前公開照片`} />}<label>公開照片網址<input name="photoUrl" type="url" defaultValue={member.photoUrl || ''} placeholder="https://..." /></label><label>或從電腦上傳新照片<input name="photoFile" type="file" accept="image/jpeg,image/png,image/webp" /></label><div className="form-note">選擇新檔案時會覆蓋網址欄位的照片。限 JPEG、PNG、WebP，最大 3 MB。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button>{canManageAll && <button type="button" className="danger-button" onClick={() => setModal({ type: 'staffDelete', id: member.id })}>永久刪除</button>}<button className="primary-button">儲存公開資料</button></footer></form><form className="modal-form line-link-form" onSubmit={(event) => linkStaffLine(event, member)}><div className="form-note"><strong>{member.lineConnected ? '目前 LINE 已串接' : '目前 LINE 尚未串接'}</strong><br />{member.lineConnected ? '輸入新的 LINE User ID 會先撤銷舊連結，再傳送測試通知並改綁。' : '只有測試通知成功送達才會完成串接。'}</div><label>新的 LINE User ID<input name="lineUserId" required pattern="U[0-9a-fA-F]{32}" minLength={33} maxLength={33} placeholder="U 開頭的 33 位 LINE User ID" /></label><footer className="modal-actions">{member.lineConnected && <button type="button" className="danger-button" onClick={() => unlinkStaffLine(member)}>僅解除目前 LINE</button>}<button className="primary-button" type="submit">{member.lineConnected ? '撤銷後改綁並傳通知' : '串接並傳送測試通知'}</button></footer></form></Modal> : null; })()}
      ...
      {modal?.type === 'settlement' && selectedAppointment && <Modal title={`回帳 ${selectedAppointment.id}`} subtitle="扣員工、扣店家、加員工、加店家；儲存後自動計算，可由管理層覆寫最終金額。" onClose={() => setModal(null)}><form className="modal-form" onSubmit={(event) => saveSettlement(event, selectedAppointment)}><div className="form-grid two"><label>扣員工<input name="discountEmployeeAmount" type="number" min="0" defaultValue={selectedAppointment.discountEmployeeAmount || 0} /></label><label>扣店家<input name="discountShopAmount" type="number" min="0" defaultValue={selectedAppointment.discountShopAmount || 0} /></label><label>加員工<input name="surchargeEmployeeAmount" type="number" min="0" defaultValue={selectedAppointment.surchargeEmployeeAmount || 0} /></label><label>加店家<input name="surchargeShopAmount" type="number" min="0" defaultValue={selectedAppointment.surchargeShopAmount || 0} /></label><label>夥伴抽成<input name="staffReturnAmount" type="number" min="0" defaultValue={selectedAppointment.staffReturnAmount || 0} /></label><label>店家回收<input name="shopRecoveryAmount" type="number" min="0" defaultValue={selectedAppointment.shopRecoveryAmount || 0} /></label></div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">儲存回帳</button></footer></form></Modal>}
      {scheduleReminderResult && <Modal title="排班通知派發結果" subtitle={`批次 ${scheduleReminderResult.batch_id}・${scheduleReminderResult.week_starts.join('、')}`} onClose={() => setScheduleReminderResult(null)} wide><div className="detail-stack"><div className="form-note">成功 {scheduleReminderResult.sent}・跳過 {scheduleReminderResult.skipped}・失敗 {scheduleReminderResult.failed}</div><div className="reminder-result-list">{scheduleReminderResult.results.map((item) => { const statusLabel = item.status === 'sent' ? '已發送' : item.status === 'failed' ? (item.reason === 'LINE 月額度已用完' ? 'LINE 月額度已用完' : 'LINE 推播未完成') : '已跳過'; return <article key={`${item.staff_id}-${item.status}-${item.reason || ''}`}><div><strong>{item.staff_name}</strong><small>{statusLabel}{item.line_uid_status ? `・${item.line_uid_status}` : ''}</small></div><small>{item.later_week_has_schedule ? '下週已有排班' : '下週尚無排班'}・{item.following_week_has_schedule ? '後週已有排班' : '後週尚無排班'}</small></article>; })}</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setScheduleReminderResult(null)}>關閉</button><button type="button" className="primary-button" disabled={scheduleReminderBusy} onClick={() => dispatchScheduleReminders(scheduleReminderResult.results.map((item) => item.staff_id))}>{scheduleReminderBusy ? '派發中…' : '重新派發'}</button></footer></div></Modal>}
      {modal?.type === 'scheduleReminderHistory' && <Modal title="排班通知紀錄" subtitle="每次派發皆保留稽核紀錄；歷史紀錄不會限制後續重新派發。" onClose={() => setModal(null)} wide><div className="detail-stack">{scheduleReminderHistory.length === 0 ? <div className="empty-state">目前沒有排班通知紀錄。</div> : <div className="reminder-result-list">{scheduleReminderHistory.map((batch) => <article key={batch.batch_id}><div><strong>{batch.batch_id}・{batch.actor_name}</strong><small>{batch.created_at ? new Date(batch.created_at).toLocaleString('zh-TW') : '—'}・成功 {batch.sent}・跳過 {batch.skipped}・失敗 {batch.failed}</small></div><small>{batch.week_starts.join('、')}</small>{batch.results.length > 0 && <div className="form-note">{batch.results.map((item) => <span key={`${batch.batch_id}-${item.staff_id}`} className="inline-actions"><strong>{item.staff_name}</strong><span>{item.status === 'sent' ? '已發送' : item.status === 'failed' ? (item.reason === 'LINE 月額度已用完' ? 'LINE 月額度已用完' : 'LINE 推播未完成') : '已跳過'}</span></span>)}</div>}</article>)}</div>}<footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>關閉</button></footer></div></Modal>}
    </main>
  );
}
