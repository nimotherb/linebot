...
export default function Home() {
  ...
  const [bulkStaffCategories, setBulkStaffCategories] = useState<string[]>([]);
  ...
  const addStaff = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const category = String(data.get('category') || staffCategories[0]?.key || '');
    if (appMode === 'live') {
      try {
  ...
  const saveStaffProfile = async (event: FormEvent<HTMLFormElement>, member: typeof staff[number]) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const category = String(data.get('category') || member.category);
    const photo = data.get('photoFile');
    try {
      if (appMode === 'live' && member.apiId) {
        ...
        setStaff((current) => current.map((item) => item.id === member.id ? { ...item, name: String(data.get('name')).trim(), category, categories: [category], photoUrl: uploaded || undefined, height: String(data.get('height') || '') || undefined, weight: String(data.get('weight') || '') || undefined, role: String(data.get('role') || '') as StaffMember['role'] || undefined, bio: String(data.get('bio') || '').trim().slice(0, 60) || undefined } : item));
      }
      setModal(null);
  ...
  const saveBulkStaffCategories = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const ids = selectedIds.staff || [];
    if (ids.length < 2 || !bulkStaffCategories.length) return notify('請至少勾選兩位員工並選擇分類。');
    try {
      await api.bulkStaffCategory(ids, bulkStaffCategories);
      const data = await api.bootstrap();
      applyBootstrap(data, 'live');
      setModal(null);
      notify('員工分類已批量更新。');
    } catch (error) {
      notify(error instanceof Error ? error.message : '批量分類更新失敗');
    }
  };
  ...
  return (
    <main className="app-shell">
      ...
      {modal?.type === 'staff' && <Modal title="新增員工" subtitle="基本資料與 LINE 串接都會保存到正式資料庫。" onClose={() => setModal(null)}><form className="modal-form" onSubmit={addStaff}><label>姓名／稱呼<input name="name" required /></label><label>手機 ID<input name="phone" inputMode="tel" placeholder="師傅手機 ID 登入使用" /></label><label>師傅分類<select name="category" defaultValue={staffCategories[0]?.key || ''}><option value="">未設定</option>{staffCategories.map((item) => <option key={item.key} value={item.key}>{item.name}</option>)}</select></label><div className="form-grid two"><label>身高（cm）<input name="height" type="number" min="100" max="250" /></label><label>體重（kg）<input name="weight" type="number" min="30" max="250" /></label></div><label>角色<select name="role" defaultValue=""><option value="">尚未設定</option><option>攻擊手</option><option>守備方</option><option>無特定</option><option>攻守兼備</option></select></label><label>師傅自我簡介（最多 60 字）<textarea name="bio" maxLength={60} rows={3} placeholder="例如：細心掌握節奏，讓每次互動都更自在。" /></label><label>公開照片網址<input name="photoUrl" type="url" placeholder="https://..." /></label><label>或從電腦上傳照片<input name="photoFile" type="file" accept="image/jpeg,image/png,image/webp" /></label><div className="form-note">若同時填網址並選擇檔案，會以上傳檔案為準；限 JPEG、PNG、WebP，最大 3 MB。</div>{/* 回帳表選擇已移除，訂單建立時使用快照與彈性金額欄位。 */}<label>LINE User ID（選填）<input name="lineUserId" pattern="U[0-9a-fA-F]{32}" minLength={33} maxLength={33} placeholder="U 開頭的 33 位 LINE User ID" /></label><div className="form-note">若填寫 LINE ID，只有測試通知成功送達才會建立並完成串接。</div><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">建立員工</button></footer></form></Modal>}

      {modal?.type === 'staffBulkCategory' && <Modal title="批量編輯員工分類" subtitle={`已選 ${selectedIds.staff?.length || 0} 位員工；可複選並疊加分類。`} onClose={() => setModal(null)}><form className="modal-form" onSubmit={saveBulkStaffCategories}><fieldset className="checkbox-field"><legend>員工分類（可複選）</legend>{staffCategories.map((item) => <label key={item.key}><input type="checkbox" checked={bulkStaffCategories.includes(item.key)} onChange={() => setBulkStaffCategories((current) => current.includes(item.key) ? current.filter((key) => key !== item.key) : [...current, item.key])} />{item.name}</label>)}</fieldset><footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>取消</button><button className="primary-button">套用分類</button></footer></form></Modal>}
