...
export default function Home() {
  ...
  const [bulkStaffCategories, setBulkStaffCategories] = useState<StaffMember['category'][]>([]);
  ...
  const saveAppointmentEdit = async (event: FormEvent<HTMLFormElement>, appointment: Appointment) => {
    ...
    if (canManageAll) {
      ...
      payload.discount_employee_amount = Number(data.get('discountEmployeeAmount') ?? appointment.discountEmployeeAmount ?? 0);
      payload.discount_shop_amount = Number(data.get('discountShopAmount') ?? appointment.discountShopAmount ?? 0);
      payload.surcharge_employee_amount = Number(data.get('surchargeEmployeeAmount') ?? appointment.surchargeEmployeeAmount ?? 0);
      payload.surcharge_shop_amount = Number(data.get('surchargeShopAmount') ?? appointment.surchargeShopAmount ?? 0);
      payload.staff_return_amount = Number(data.get('staffReturnAmount') ?? appointment.staffReturnAmount ?? 0);
      payload.shop_recovery_amount = Number(data.get('shopRecoveryAmount') ?? appointment.shopRecoveryAmount ?? 0);
    }
    try {
  ...
  const saveSettlement = async (event: FormEvent<HTMLFormElement>, appointment: Appointment) => {
    ...
    // Leaving the calculated totals untouched while changing A/B/C/D means
  ...
  const saveBulkStaffCategories = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const values = bulkStaffCategories.map((label) => label === '直男師傅' ? 'straight' : label === '雙性師傅' ? 'bisexual' : 'gay');
      await api.bulkStaffCategory(ids, values);
      const data = await api.bootstrap();
      applyBootstrap(data, 'live');
      setModal(null);
      notify('員工分類已批量更新。');
  ...
  const renderStaff = () => (
    <section className="panel table-panel">
      ...
      <div className="staff-card-grid">{pagedStaff.map((member) => <article className={member.status === '暫時退役' ? 'staff-card retired' : 'staff-card'} key={member.id}>
        ...
        {/* 回帳表選擇已移除；訂單建立時依快照與 A/B/C/D 彈性欄位自動計算。 */}
  ...
  return (
    <main className="app-shell">
      ...
      {modal?.type === 'appointmentDetail' && selectedAppointment && <Modal title={selectedAppointment.id} subtitle={`${selectedAppointment.date}　${selectedAppointment.start}–${selectedAppointment.end}`} onClose={() => setModal(null)}><div className="detail-stack"><div className="detail-hero"><div><span>客戶</span><strong>{selectedAppointment.customer}</strong><small>{selectedAppointment.customerSerial ? `客人識別 ${selectedAppointment.customerSerial}・` : ''}{selectedAppointment.phone}</small></div><StatusPill status={selectedAppointment.status} /></div><dl><div><dt>師傅</dt><dd>{selectedAppointment.staff}</dd></div><div><dt>服務方案</dt><dd>{selectedAppointment.service}</dd></div><div><dt>優惠</dt><dd>{selectedAppointment.promotionName || '未使用'}</dd></div><div><dt>場地</dt><dd>{selectedAppointment.location}・{selectedAppointment.room}</dd></div><div><dt>應收金額</dt><dd>{formatCurrency(selectedAppointment.total)}</dd></div><div><dt>付款狀態</dt><dd>{selectedAppointment.payment}</dd></div><div><dt>回帳調整</dt><dd>A −{formatCurrency(selectedAppointment.discountEmployeeAmount || 0)}／B −{formatCurrency(selectedAppointment.discountShopAmount || 0)}／C ＋{formatCurrency(selectedAppointment.surchargeEmployeeAmount || 0)}／D ＋{formatCurrency(selectedAppointment.surchargeShopAmount || 0)}</dd></div><div><dt>師傅回帳／店家回收</dt><dd>{formatCurrency(selectedAppointment.staffReturnAmount || 0)}／{formatCurrency(selectedAppointment.shopRecoveryAmount || 0)}</dd></div></dl>{selectedAppointment.note && <div className="detail-note"><strong>客服備註</strong><p>{selectedAppointment.note}</p></div>}<div className="status-actions">{canEditAppointments && <button onClick={() => setModal({ type: 'appointmentEdit', id: selectedAppointment.id })}>編輯整張訂單</button>}{canManageAll && <button onClick={() => setModal({ type: 'settlement', id: selectedAppointment.id })}>編輯回帳 A／B／C／D</button>}{canEditAppointments && selectedAppointment.status !== '已完成' && <button onClick={() => setModal({ type: 'checkout', id: selectedAppointment.id })}>記錄付款並完成</button>}</div></div></Modal>}
